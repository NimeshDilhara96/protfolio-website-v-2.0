...
const downloads = [
    ...
    {
        icon: <FaAndroid className="text-4xl text-[#34B27B]" />,
        title: 'Note App',
        desc: 'My new Project.',
...
function Download() {
    return (
        <>
            <Navbar />
            <div className="min-h-screen bg-gradient-to-br from-black to-[#11181C] relative overflow-hidden">
                {/* Background decoration */}
                <div className="absolute inset-0 overflow-hidden">
                    <div className="absolute top-20 left-10 w-32 h-32 bg-[#34B27B]/10 rounded-full blur-3xl animate-pulse"></div>
                    <div className="absolute bottom-20 right-10 w-40 h-40 bg-[#34B27B]/10 rounded-full blur-2xl animate-pulse" style={{animationDelay: '1s'}}></div>
                </div>

                <main className="relative z-10 container mx-auto px-4 pt-24 pb-16">
                    {/* Page Title */}
                    <div className="text-center mb-16">
                        <h2 className="text-4xl md:text-5xl font-bold text-[#F8F9FA] mb-4 tracking-tight flex items-center justify-center gap-4">
                            <FaDownload className="text-[#34B27B]" />
                            Downloads
                        </h2>
                        <div className="w-20 h-1 bg-[#34B27B] mx-auto rounded-full"></div>
                        <p className="text-[#F8F9FA]/70 text-lg mt-6 max-w-2xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-4xl mx-auto">
                        {downloads.map((item, idx) => (
                            ...
                            >
                                {/* Icon */}
                                <div className="flex items-center justify-center w-16 h-16 mx-auto mb-6 rounded-xl bg-[#11181C] shadow-lg group-hover:scale-110 transition-transform duration-300">
                                    {item.icon}
                                </div>

                                {/* Content */}
                                <div className="text-center">
                                    <h3 className="text-xl font-bold text-[#F8F9FA] mb-3 group-hover:text-[#34B27B] transition-colors duration-300">
                                        {item.title}
                                    </h3>
                                    <p className="text-[#F8F9FA]/70 text-sm mb-6 leading-relaxed">
                                        {item.desc}
                                    </p>
                                ...
                                {/* Bottom glow effect */}
                                <div className="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#34B27B]/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                            </div>
                        ))}
                    </div>
